...
that.$element.css({"position":"fixed","left":($(window).width()/ 2 ) - ( that.w / 2 ),
"top":($(window).height()/ 2 ) - ( that.h / 2 ) - resizedOffs
});that.enforceFocus();},cloneSize:function()
{var that=this;that.$clone=that.$element.filter(':first').clone().css({'position':'absolute','top':-2000,'display':'block','visibility':'visible','opacity':100}).removeClass('fade').appendTo('body');that.h=that.$clone.height();that.w=that.$clone.width();that.$clone.remove();that.$element.css({"position":"fixed","left":($(window).width()/ 2 ) - ( that.w / 2 ),
